...
mysql -D play_with_languages -u root -p < lado_db.sql

mkdir phrase_images_related
chown www-data:www-data phrase_images_related

dpkg -l | grep -E '^ii' | grep -q php.*gd && echo "php-gd module seems to be installed! Great.." || echo "Installing php-gd module..." && apt-get install php-gd -y
